...
public class HashTableQuadraticProbing<K, V> implements Iterable<K> {
    ...
    private final K TOMBSTONE = (K) (new Object());  //Special marker token used to indicate the deletion of a key-value pair
    ...
    private boolean containsFlag = false;  //flag used to indicate whether an item was found in the hash table
    ...
    //Converts a hash value to an index.Essentially, this strips the negative sign and places the hash value in the
    // domain [0, capacity]
    ...
    //Place a key-value pair into the hash-table.If the value already exists inside the hash-table then the value is updated
    public V insert(K key, V val) {
        ...
        do {
            //The current slot was obviously deleted
            if (keyTable[i] == TOMBSTONE) {
                ...
                //the key we're trying to insert already exists in the hash-table,so update its value with the most recent value
                ...
                if (j == 1) {
                    ...
                    //Previously seen deleted bucket. Instead of inserting the new element at i where the null element
    ...
    public boolean hasKey(K key) {
        get(key);//sets the 'containsFlag'
        return containsFlag;
    }
    //Get the value associated with the input key.NOTE: returns null if the value is null AND also returns null if the
    // key does not exists
    ...
    public V get(K key) {
        ...
        //Starting at the original hash index quadratically probe until we find a spot where our key is or we hit a
        // null element in which case our element does not exist
        do {
            //ignore deleted cells, but record where the first index of deleted cell is found to perform lazy relocation
            //later
            if (keyTable[i] == TOMBSTONE) {
                if (j == -1) j = 1;

                //We hit a non null key, perhaps its the one we're looking for.
            } else if (keyTable[i] != null) {
                //The key we want is in the hash-table!
                if (keyTable[i].equals(key)) {
                    containsFlag = true;

                    //if j != -1 this means we previously encountered a deleted cell.we can perform an optimization by
                    // swapping the entries in cells i and j so that the next time we search for this key it will be
    ...
    //Removes a key from the map and returns the value .NOTE: returns null if the value is null and also returns null
    ...
    public V remove(K key) {
        ...
        //Starting at the hash index quadratically probe until we find a spot where our key is or we hit a null element
        // in which case our element does not exist
        for (; ; i = normalizeIndex(hash + P(x++))) {

            if (keyTable[i] == TOMBSTONE) continue;  //Ignore deleted cells
            if (keyTable[i] == null) return null;  //Key was not found in hash-table
            //The key we want to remove is on the hash-table
            if (keyTable[i].equals(key)) {
                keyCount--;
                modificationCount++;
                V oldValue = valueTable[i];
    ...
    private void resizeTable() {
        ...
        //Reset the key count and buckets used since we are about to re insert all the keys into the hash-table
        keyCount = usedBuckets = 0;
    ...
    public java.util.Iterator<K> iterator() {
        //Before the iteration begins record the number of modifications done to the hash-table.This value should not
        // change as we iterate otherwise a concurrent modification has occurred
    ...
    //Return a String version of the hash-table
    @Override
