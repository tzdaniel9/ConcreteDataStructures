...
public class MinIndexedDHeap<T extends Comparable<T>> {
    ...
    //The position Map (pm) maps Key Indexes (ki) to where the position of that key is represented in the priority queue
    // in the domain [0, sz].
    public final int[] pm;
    //The inverse Map (im) stores the indexes of the keys in the range [o, sz] which makes up the priority queue.It
    // should be noted that 'im' and'pm' are inverses of each other, so: pm[im[i]] = i
    public final int[] im;
    ...
    private final int[] child, parent;   //Lookup arrays to track the child/parent indexes of each node.
    private int sz;   //Current number of elements in the heap

    //Initializes a D-array heap with a maximum capacity of maxSize.
    ...
    //Recursively checks if this heap is a main heap. This method is used for testing purposes to validate the heap invariant
